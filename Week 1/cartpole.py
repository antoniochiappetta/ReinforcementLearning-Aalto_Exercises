...
def parse_args(args=sys.argv[1:]):
    ...
    parser.add_argument("--balance_point", type=int, default=0,
                        help="Arbitrary point where to balance the pole")
...
def train(agent, env, train_episodes, early_stop=True, render=False,
    ...
    for episode_number in range(train_episodes):
        ...
        while not done:
            ...
            # reward = new_reward_center(observation)
            # reward = new_reward_arbitrary(observation)
            reward = new_reward_velocity(observation)
...
def new_reward_center(state):
    desired_x = 0
    pos = state[0]
    diff = abs(pos-desired_x)
    delta_x = 4.8  # episode terminate when car position is more than 2.4 from the center
    return 1-diff/delta_x

def new_reward_arbitrary(state):
    desired_x = args.balance_point
    pos = state[0]
    diff = abs(pos-desired_x)
    delta_x = 4.8  # episode terminate when car position is more than 2.4 from the center
    return 1-diff/delta_x

def new_reward_velocity(state):
    vel = state[1]
    return 1 + abs(vel)
