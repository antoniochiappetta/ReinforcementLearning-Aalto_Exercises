...
class SailingGridworld(object):
    ...
    def __init__(self, rock_penalty=-2, harbour_reward=10, wind_p=0.1, discount_factor=1, threshold=0):
        self.wind_x = 5, 11
        self.wind_y = 6, 8
        self.rocks1_x = 5, 11
        self.rocks2_x = 5, 11
    ...
    def get_state_value(self, value_est, state):
        value, action = max(
                map(
                    lambda x: self.get_state_action_value(value_est, state, x),
                    range(self.NO_ACTIONS)
                ), key=lambda x: x[0]
            )
        return value, action

    def get_state_action_value(self, value_est, state, action):
        possible_transitions = self._get_possible_transitions(state, action)
        return reduce(lambda a, b: a+b, map(
            lambda x: self.get_transition_value(value_est, x),
            possible_transitions
        )), action

    def get_transition_value(self, value_est, transition):
        prob = transition[3]
        terminate = transition[2]
        reward = transition[1]
        next_state = transition[0]
        if terminate is True:
            return 0
        else:
            return prob * (reward + self.discount_factor * value_est[next_state[0], next_state[1]])

    def state_value_iteration(self, value_est, policy):
        delta_value_est = np.zeros((self.w, self.h))
        for state, value in np.ndenumerate(value_est):
            # New optimal value and next action for state s
            value, action = self.get_state_value(value_est, state)
            # Compute delta value with old value function for state s
            delta_value_est[state[0], state[1]] = abs(value - value_est[state[0], state[1]])
            # Update rule for value function
            value_est[state[0], state[1]] = value
            # Update rule for policy function
            policy[state[0], state[1]] = action
        # Check convergence condition using delta values computed between
        # current and previous value iteration
        converged = (np.amax(delta_value_est) < self.threshold)
        return value_est, policy, converged
